...
int ll_count(LinkedList* this, int (*fn)(void* element, int enteroCriterio), int enteroCriterio)
{
    Node* pNodeAux;
    int contadorSegunfn=0;
    int flagEstado=ERROR;
    ...
    if(this!=NULL && fn!=NULL)
    {
        ...
        if(pNodeAux!=NULL)
        {
            flagEstado=TODOOK;
            do{
                contadorSegunfn+=fn(pNodeAux->pElement, enteroCriterio);
                pNodeAux=ll_getNextNode(this);
            }while(pNodeAux!=NULL);
        }
    }
    if(flagEstado==TODOOK)
        return contadorSegunfn;
    else
        return flagEstado;
}
