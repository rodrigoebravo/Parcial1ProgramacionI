...
/** \brief Imprime todos los elementos de la estructura empleado
 * \param this Employee* puntero a la estructura
 * \return
 */
void Employee_print(void* this){
    int bufferId;
    char bufferNombre[120];
    int bufferHoras;
    int bufferSueldo;
    if(this != NULL){
...
/** \brief filta empleados x sueldo
 * \param this Employee* puntero a la estructura
 * \return devuelve 1 si cumple la condicion
 */
int Employee_filtroSueldo(void* this){
    int retorno=0;
    int max=11000;
    int min=9000;
    int bufferSueldo;
    if(this!=NULL && min<max){
        Employee_getSueldo(this,&bufferSueldo);
        if(bufferSueldo>min &&bufferSueldo<max){
            retorno=1;
        }
    }
    return retorno;
