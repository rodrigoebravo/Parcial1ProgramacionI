...
int inf_contarCantidadVentasPorCriterio(void* pElement, void* valorCriterio)
{
    ...
    if((pElement!=NULL &&
        Venta_getCantidad(pElement, &cantidad)==TODOOK &&
        Venta_getPrecioUnitario(pElement, &precioUnitario)==TODOOK &&
        (cantidad * precioUnitario)>*monto) ||
       (pElement!=NULL &&
        Venta_getCodigoProducto(pElement, codigoProducto)==TODOOK &&
        strcmp(codigoProducto, valorCriterio)==0)
       )
        valorRetorno=1;
    return valorRetorno;
}
